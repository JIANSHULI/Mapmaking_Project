...
double fractionOfModesToThrowOut = .03;
bool ignoreTopAndLeftEdge = false;
double throwOutModesWithObservationTimeRationsGreaterThanThis = 5;
...
void maskVeryNoisyCells(vector< vector< vector<double> > >& observationTimes){
	...
	cout << endl << int(round(fractionOfModesToThrowOut*xBins*yBins)) << " modes thrown out for large RMSs." << endl;
	...
	cout << highRatioModesThrownOut << " modes thrown out due to highly inconsistant observation times." << endl << endl;
}
