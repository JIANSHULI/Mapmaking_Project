...
Specs *s;
double effectiveArea, systemTemperature, observationTime, fLength, xyLength, fStart, fractionOfModesToThrowOut, throwOutModesWithObservationTimeRationsGreaterThanThis;
...
void loadSpecs(string dataSpecsFilename, string cubeParametersFilename, string NSpecsFilename){
	...
	infile >> dummy >> fractionOfModesToThrowOut;
...
void maskVeryNoisyCells(vector< vector< vector<double> > >& observationTimes){
	//Find Modes with Biggest RMS
	vector<CVector> deconvolvedTemps0 = loadAllDeconvolvedFields(0);
	vector<CVector> deconvolvedTemps1 = loadAllDeconvolvedFields(1);
	int centerFieldUnmaskedBins = 0;
	for (int i = 0; i < xBins; i++){
		for (int j = 0; j < yBins; j++){
			for (int k = 0; k < fBins; k++){
				double temp0 = deconvolvedTemps0[centerField].real[i*yBins*fBins + j*fBins + k];
				double temp1 = deconvolvedTemps1[centerField].real[i*yBins*fBins + j*fBins + k];
				if (!(temp0 == 0 && temp1 == 0)) centerFieldUnmaskedBins++;
			}
		}
	}
	vector<CVector> allSynthBeams;	
	for (int f = 0; f < fields; f++){
		CVector uvweightsHere0 = loadFileIntoCVector(f, 0, "uvweights").ijk2uvk();
		CVector uvweightsHere1 = loadFileIntoCVector(f, 1, "uvweights").ijk2uvk();
		for (int n = 0; n < nElements; n++){
			uvweightsHere0.real[n] = (uvweightsHere0.real[n]+uvweightsHere1.real[n])/2;
			uvweightsHere0.imag[n] = (uvweightsHere0.imag[n]+uvweightsHere1.imag[n])/2;
		}
		allSynthBeams.push_back(uvweightsHere0);
	}

	vector<vector<double> > kPerpRMS(xBins, vector<double>(yBins,0));
	vector<CVector> allFields0 = deconvolvedTemps0;
	vector<CVector> allFields1 = deconvolvedTemps1;
	for (int f = 0; f < fields; f++){
		allFields0[f] = allFields0[f].ijk2uvk();
		allFields1[f] = allFields1[f].ijk2uvk();
	}
	for (int f = 0; f < fields; f++){
		for (int k = 0; k < fBins; k++){	
			for (int i = 0; i < xBins; i++){
				for (int j = 0; j < yBins; j++){
					kPerpRMS[i][j] += (pow(allFields0[f].real[i*yBins*fBins + j*fBins + k],2) + pow(allFields0[f].imag[i*yBins*fBins + j*fBins + k],2))/(centerFieldUnmaskedBins/xBins/yBins * fields);
				}
			}
		}
	}
	vector<vector<double> > kPerpRMSCopy = kPerpRMS;

	//Mark as "unobserved" modes with RMS in the top 5% of RMSs
	
	for (int n = 0; n < int(round(fractionOfModesToThrowOut*xBins*yBins)); n++){
		double largestRMS = 0;
		int iLargest;
		int jLargest;
		for (int i = 0; i < xBins; i++){
			for (int j = 0; j < yBins; j++){
				if (kPerpRMS[i][j] > largestRMS){
					largestRMS = kPerpRMS[i][j];
					iLargest = i;
					jLargest = j;
				}
			}
		}
		kPerpRMS[iLargest][jLargest] = 0;
		for (int k = 0; k < fBins; k++){
			observationTimes[iLargest][jLargest][k] = 0;
		}
	}
	cout << endl << int(round(fractionOfModesToThrowOut*xBins*yBins)) << " modes thrown out for large RMSs (top " << fractionOfModesToThrowOut*100 << "%)." << endl;
	...
	for (int i = 0; i < xBins; i++){
		for (int j = 0; j < yBins; j++){
			...
			if (largestObs/smallestObs > throwOutModesWithObservationTimeRationsGreaterThanThis){
