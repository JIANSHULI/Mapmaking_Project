...
CVector loadAverageUVweights(bool fullField){
	CVector uvweights0 = loadFileIntoCVector(centerField, 0, "psf",fullField);
	CVector uvweights1 = loadFileIntoCVector(centerField, 1, "psf",fullField);
...
void throwOutLeastObservedModes(vector< vector< vector<double> > >& noiseCovarianceMatrixDiagonal){
	...
	for (int i = 0; i < xBins; i++){
		for (int j = 0; j < yBins; j++){
			for (int k = 0; k < fBins; k++){
				if (noiseCovarianceMatrixDiagonal[i][j][k] > bestMode*throwOutModesThisMuchWorseThanTheBest) noiseCovarianceMatrixDiagonal[i][j][k] = -1;
			}
		}
	}
}
