...
trueSky = load([prefixes{n} 'true_sky.dat']);
PSF = load([prefixes{n} 'PSF.dat']);
map = load([prefixes{n} 'map.dat'])/tempFactor;
directMap = load([prefixes{n} 'directMultiplicationMap.dat']);%/tempFactor;
...
axes(ha(6));
scatter(ras, decs, mSize, directMap-convolvedSky, 'fill','Marker','d','MarkerEdgeColor','none');
%scatter(ras, decs, mSize, Dmatrix, 'fill','Marker','d','MarkerEdgeColor','none');
colorbar; title('Direct Map from A^t - Convolved Sky');
xlabel('Right Ascension'); ylabel('Declination');
mapRange = get(gca,'CLim');
axis square
set(gca,'XLim',raRange); set(gca,'YLim',decRange);
...
disp(['Map vs. Direct Map Error is ' num2str(norm(map-directMap)/norm(directMap))])
disp(['Direct Map vs. Convolved Sky Error is ' num2str(norm(directMap-convolvedSky)/norm(convolvedSky))])