...
void loadSpecs(){
	...
	infile >> dummy >> facetRA;
	infile >> dummy >> facetDec;
	infile >> dummy >> angularResolution;
	infile >> dummy >> facetSize;
	infile >> dummy >> PSFextensionBeyondFacetFactor;
	infile.close();
...
void saveResultsToFile(vector< vector<double> >& facetMap){
	...
	stringstream ss;
	ss << resultsFolder << "Map_at_" << facetRA << "_RA_" << facetDec << "_Dec_" << freq << "_MHz_" << angularResolution << "_Deg_Res_PSF_" << PSFextensionBeyondFacetFactor << ".dat";
...
int main(int argc, char *argv[]){ 
	...
	double Frequencies[] = {125.586, 150.195, 175.196};
