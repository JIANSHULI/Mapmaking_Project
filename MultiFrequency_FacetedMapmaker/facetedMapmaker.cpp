...
int extendedIndexOfCentralPixel = 0;
...
void loadSpecs(){
	...
	primaryBeamFWHM = primaryBeamFWHM * 150 / freq;
...
vector< vector<complex> > loadVisibilities(vector<cartVec>& baselines, vector<double>& LSTs){	
	...
	for (int n = 0; n < nBaselines; n++){
		...
		ss << visibilitiesFolder  << -baselines[n].y << "_m_south_" << baselines[n].x << "_m_east_" << baselines[n].z << "_m_up_" << polarization << "_pol_" << 150.195 << "_MHz.dat";
...
vector<int> computeHealpixIndices(bool extended){
	...
	if (extended){
		equaPoint PSFcenterEquaPoint = facetCenterEquaPoint;
		
		PSFcenterEquaPoint.ra = PSFcenterEquaPoint.ra + 2.5*2*pi/360;

		int HPindexOfCentralPixel = sampleHealpixMap.ang2pix(PSFcenterEquaPoint.toHealpixPointing());
		for (int n = 0; n < nPixelsExtended; n++){
			if (HPindexOfCentralPixel == healpixIndices[n]) extendedIndexOfCentralPixel = n;
		}
...
void addSnapshotPSF(vector<double>& PSF, vector< vector<complex> >& KAtranspose, vector<double>& Ninv, vector<int>& mapOfIndicesInExtendedIndexVector){
	//for (int n = 0; n < nPixels; n++){
		for (int nx = 0; nx < nPixelsExtended; nx++){
			for (int b = 0; b < nBaselines; b++){
				PSF[nx] = PSF[nx] + 2 *(KAtranspose[nx][b] * KAtranspose[extendedIndexOfCentralPixel][b].conj() * Ninv[b]).re;
			}
		}
	//}
}
...
//For the single PSF, multi-frequency test
void normalizePSF(vector<double>& PSF){
	double normalization = 1.0/PSF[extendedIndexOfCentralPixel];
	for (int n = 0; n < nPixelsExtended; n++){
		PSF[n] = PSF[n] * normalization;
	}
}
...
int main(int argc, char *argv[]){
	...
	vector<double> PSF(nPixelsExtended,0.0);
	...
	normalizePSF(PSF);
	exportVector(PSF, nPixelsExtended, PSFfilename);
